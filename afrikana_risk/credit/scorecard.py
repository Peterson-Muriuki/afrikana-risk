...
def _woe_iv(
    ...
    agg["woe"] = np.log((agg["dist_events"] + eps) / (agg["dist_nonevents"] + eps))
...
class ScorecardBuilder:
    ...
    def fit(
        ...
        cols = feature_cols or [
            c
            for c in df.select_dtypes(include=["number", "category", "object"]).columns
            if c != target
        ]
        ...
        self._selected_vars = [
            v for v, iv in self._iv.items() if self.config.min_iv <= iv <= 0.5
        ]
    ...
    def score(self, df: pd.DataFrame) -> pd.Series:
        ...
        X_woe = self._woe_encode(df)
        log_odds = (
            X_woe.values @ np.array([self._lr_coefs[v] for v in self._selected_vars])
        ) + self._lr_intercept
    ...
    def iv_summary(self) -> pd.DataFrame:
        ...
        return (
            pd.DataFrame({"variable": list(self._iv.keys()), "iv": list(self._iv.values())})
            .assign(
    ...
    def _extract_bin_edges(woe_tbl: pd.DataFrame) -> np.ndarray | None:
        ...
        bins = woe_tbl["bin"]
        if bins.empty or not hasattr(bins.iloc[0], "left"):
            return None

        edges = sorted(set([b.left for b in bins] + [bins.iloc[-1].right]))
        edges[0] = -np.inf
        edges[-1] = np.inf
